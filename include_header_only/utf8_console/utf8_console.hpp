...
int u8c_wcwidth(int ucs) {
  ...
  if (intable(ZERO_WIDTH, sizeof(ZERO_WIDTH) / sizeof(struct width_interval),
              ucs))
    return 0;

  return intable(WIDE_EASTASIAN,
                 sizeof(WIDE_EASTASIAN) / sizeof(struct width_interval), ucs)
             ? 2
             : 1;
}
#endif
...
std::streambuf::int_type win_utf8_streambuf::underflow() {
  ...
  auto ret = ReadConsoleW(GetStdHandle(STD_INPUT_HANDLE), _wide_buffer.get(),
                          _wide_buf_size, &read_len, NULL);
  ...
  int size = WideCharToMultiByte(CP_UTF8, 0, _wide_buffer.get(), read_len,
                                 _buffer.get(), _u8_buf_size, NULL, NULL);
...
void initialize_utf8_console_windows() {
  ...
  if (!global_win_utf8_streambuf)
    global_win_utf8_streambuf =
        std::make_unique<win_utf8_streambuf>(2048, true);
